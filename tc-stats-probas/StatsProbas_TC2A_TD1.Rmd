...
- Deux cas particulier : 
  - *Binaire* : 2 valeurs possibles uniquement (Ex : oui/non, présence/absence...)
  - *Ordinale* : Modalités devant être triées dans un ordre spécifique (Ex : mois, sentiment...)
...
```{r qt-graph-hist, warning=FALSE, message=FALSE}
library(cowplot)
...
```{r qt-graph-box, warning=FALSE, message=FALSE}
library(cowplot)
ggplot(diamonds, aes(price, "")) + 
...
kbl(tib, digits = 2, format = "html") %>% column_spec(1:5, width = "5em") %>%
  kable_styling(position = "center")
```
...
```{r qtql-graph-bis, warning=FALSE, message=FALSE}
