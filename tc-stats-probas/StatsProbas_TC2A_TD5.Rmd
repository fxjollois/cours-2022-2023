...
Lorsque $n$ est grand (supérieur à 1000), on définit un intervalle de confiance de la moyenne avec une probabilité $\alpha$ avec la formule suivante :
$$
  \left[
...
Lorsque $n$ est inférieur à 1000, avec une probabilité $\alpha$, un utilise la formule suivante :
$$
  \left[
  ...
  \right]
$$
où $t_{\alpha/2}$ représente la valeur dans la table de la loi *Student*, à $n-1$ degré de liberté, telle que $P(X > t_{\alpha/2}) = \frac{\alpha}{2}$).
...
Valeurs importantes de la table de la loi Normale et de la loi de *Student* (selon les degrés de liberté)

```{r}
v = c(0.001, 0.005, 0.01, 0.025, 0.05, 0.10)
knitr::kable(tibble(`P(X < u)` = 1 - v, 
                    `u (loi Normale N(0,1))` = qnorm(1 - v),
                    `t (loi Student 49)` = qt(1 - v, 49),
                    `t (loi Student 119)` = qt(1 - v, 119),
                    `t (loi Student 299)` = qt(1 - v, 299)
                    ),
             digits = 3) %>%
  kable_styling()
